...
<Window
    ...
    Title="Bulk Process - Folder Path Validator" FontWeight="Bold" Height="800" Width="900" WindowStartupLocation="CenterScreen" ResizeMode="CanMinimize">
...
$servers = Get-Content $txtFolderPath.Text -ErrorAction Stop


foreach ($server in $servers) {


$TempServer = $server.Trim()


$ACLDetails = Get-ADUser -Server "Nmea.abb.com" -Filter { Name -eq $TempServer} -ErrorAction Stop

$Groups = (Get-ADUser -Server "Nmea.abb.com" -Filter { Name -eq $TempServer } –Properties MemberOf | Select-Object MemberOf).MemberOf | Format-List | out-string
...
$results | export-csv -Append -Path "\\XE-S-JUMP005.xe.abb.com\D$\CIT_BulkProcessOutPut\FolderPathValidator\GetSID.csv" -NoTypeInformation
    

}


                            $txtboxFolderPath.text = "Successful - \\XE-S-JUMP005.xe.abb.com\D$\CIT_BulkProcessOutPut"  

                            $textBlock.Background = "#00FF00"

                            $btnSubmit.IsEnabled = $false 

                            $textBlock.text=  "Successful - \\XE-S-JUMP005.xe.abb.com\D$\CIT_BulkProcessOutPut"  
