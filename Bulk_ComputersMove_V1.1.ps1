...
<Window
    ...
    Title="Bulk Process - Computer OU Move" FontWeight="Bold" Height="800" Width="900" WindowStartupLocation="CenterScreen" ResizeMode="CanMinimize">
...
{ 
    ...
    ( 
        ...
        [Alias('LogPath')] 
        [string]$Path='C:\Logs\ComputerMove_Bulk.log', 

        [Parameter(Mandatory=$false)] 
        [Alias('LogPath2')] 
        [string]$Path2='E:\Logs\ComputerMove_Bulk.log', 
...
$inp = Import-Csv $txtFolderPath.Text
...
Write-Host $x.name
 ...
 Get-ADComputer $x.name | Move-ADObject -TargetPath $x.TargetPath -ErrorAction Stop

 $output +=  " $Count - Computer Source OU- $x.name
                        Moved Successfully
                        Computer Destination OU- $x.TargetPath
 ...
 $txtboxFolderPath.text = $output
   

  $textBlock.text  = "$Count - computers are moved successfully"               
...
$x.name | Out-File "E:\CIT\CSV\Test_ComputerMove_Error.csv" -Append
