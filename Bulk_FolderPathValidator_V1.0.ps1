...
$ACLDetails =  Get-Acl $TempServer -ErrorAction SilentlyContinue| Format-List | out-string 


if( $ACLDetails.Length -eq 0)
...
$TempOutPut += $ACLDetails

$details = @{            
                FolderPath     = $TempServer
                
                ACLDetails = $ACLDetails

PermissionIssueOrFolderPathDoesNotExist = $Issue 
                             
                
        }                           
     
...
$results | export-csv -Append -Path "\\XE-S-JUMP005.xe.abb.com\D$\CIT_BulkProcessOutPut\FolderPathValidator\FolderPathValidator.csv" -NoTypeInformation
...
$txtboxOutput.text = "Error : " + $_.Exception.Message + $tempUserHomeDrive
